...
export default {
			...
			switch (commandData.name) {
				case "ping": {
					const timestamp = new Date(
						Number(interaction.id) / 4194304 + 1420070400000,
					);
	...
	async scheduled(_event, env, _ctx) {
		...
		const currentBirthdays = results.filter(
			(birthday) =>
				birthday.birthday_month === todayMonth &&
				birthday.birthday_day === todayDay,
		);
		const inactiveBirthdays = activeBirthdays.filter(
			(birthday) =>
				!currentBirthdays.find(
					(currentBirthday) => currentBirthday.user_id === birthday.user_id,
				),
		);
		for (const birthday of inactiveBirthdays) {
			...
			const updateStmt = env.DB.prepare(
				"UPDATE birthdays SET active = ? WHERE user_id = ?",
			);
			await updateStmt.bind(false, birthday.user_id).run();
		}
		const newBirthdays = currentBirthdays.filter(
			(birthday) =>
				!activeBirthdays.find(
					(activeBirthday) => activeBirthday.user_id === birthday.user_id,
				),
		);
...
async function handleBirthdayCommand(
	...
	const subcommand: $TsFixMe = commandData.options.find(
	...
	switch (subcommand.name) {
		case "set": {
			const monthOption = subcommand.options.find(
				(option: $TsFixMe) => option.name === "month",
			);
			const dayOption = subcommand.options.find(
				(option: $TsFixMe) => option.name === "day",
			);
			...
			if (!isValidDate(monthOption.value, dayOption.value)) {
				return jsonResponse({
					...
					data: {
						content:
							"Invalid date provided. Please enter a valid month and day.",
						flags: 64,
					},
				});
			}
			...
			await insertStmt
				.bind(userId, monthOption.value, dayOption.value, false)
				.run();
			const response = await fetch(
