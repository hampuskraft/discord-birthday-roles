  ...
  async scheduled(_event, env, _ctx) {
    ...
    for (const birthday of newBirthdays) {
      let response = await fetch(
        `https://discord.com/api/v10/guilds/${env.DISCORD_GUILD_ID}/members/${birthday.user_id}/roles/${env.DISCORD_ROLE_ID}`,
        {method: 'PUT', headers: {Authorization: `Bot ${env.DISCORD_TOKEN}`}},
      );
      if (!response.ok) {
        console.error(await response.text());
      }
      ...
      response = await fetch(`https://discord.com/api/v10/channels/${env.DISCORD_CHANNEL_ID}/messages`, {
        ...
        body: JSON.stringify({content: `<@${birthday.user_id}> Happy birthday! 🎉`}),
      });
      if (!response.ok) {
        console.error(await response.text());
      }
...
async function handleBirthdayCommand(
  ...
  const subcommand: any = commandData.options.find((option) => option.type === 1);
  ...
  switch (subcommand.name) {
    case 'set': {
      const monthOption = subcommand.options.find((option: any) => option.name === 'month');
      const dayOption = subcommand.options.find((option: any) => option.name === 'day');
